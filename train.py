...
label_dir = 'data_object_image_2/training/label_2/'
...
def orientation_loss(y_true, y_pred):
    # y_true and y_pred are expected to be in radians
    sin_true = tf.sin(y_true)
    cos_true = tf.cos(y_true)
    
    sin_pred = tf.sin(y_pred)
    cos_pred = tf.cos(y_pred)
    
    # Compute squared differences between sine and cosine components
    sin_loss = tf.square(sin_true - sin_pred)
    cos_loss = tf.square(cos_true - cos_pred)
    
    # Final loss is the mean of both components
    return tf.reduce_mean(0.5 * (sin_loss + cos_loss))
